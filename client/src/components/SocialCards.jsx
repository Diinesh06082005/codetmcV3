...
function getHeightMultiplier(width) {
  ...
  const available = (typeof window !== "undefined" ? window.innerHeight : 800) * 0.7;
...
const ARROW_CLASSES =
  "relative flex items-center justify-center rounded-full border-[1.5px] border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5 backdrop-blur-[16px] text-black/60 dark:text-white/80 cursor-pointer shrink-0 z-30 outline-none shadow-[0_4px_20px_rgba(0,0,0,0.1)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.4)] hover:border-violet-500/50 dark:hover:border-violet-400/60 hover:text-violet-400 active:opacity-70 transition-colors duration-300 before:content-[''] before:absolute before:inset-[3px] before:rounded-full before:border before:border-black/[0.04] dark:before:border-white/[0.04] before:pointer-events-none";
      ...
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem]">
          {cards.map((card, index) => {
            const cardContent = (
              <div className="relative w-full h-full overflow-hidden rounded-3xl bg-slate-950/90 border border-white/10 shadow-2xl group/card">
                {card.imgUrl ? (
                  <img
                    src={card.imgUrl}
                    loading="lazy"
                    alt={card.alt || card.title || `Card ${index}`}
                    className="absolute inset-0 w-full h-full object-cover z-10 transition-transform duration-500 group-hover/card:scale-110"
                  />
                ) : (
                  <div
                    className={`absolute inset-0 bg-gradient-to-br ${
                      card.gradient || "from-violet-900/80 via-slate-950 to-indigo-950"
                    } z-10`}
                  />
                )}

                {/* Dark gradient overlay for title text visibility */}
                ...
                <div className="absolute bottom-0 inset-x-0 p-5 z-30 flex flex-col justify-end space-y-2 text-left">
                  {card.tag && (
                    <span className="inline-block px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold uppercase tracking-wider bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 w-max">
                      {card.tag}
                    </span>
                  )}
                  {card.title && (
                    <h3 className="text-base font-extrabold text-white tracking-tight leading-snug drop-shadow-md">
                      {card.title}
                    </h3>
                  )}
                  {card.description && (
                  ...
                  {card.actionText && (
                    <div className="pt-2">
                      <span className="inline-flex items-center gap-1 text-[11px] font-bold text-cyan-400 group-hover/card:text-cyan-300 transition">
      ...
      {needsPagination && (
        <div className="flex items-center justify-center gap-4 mt-4 md:mt-6 z-30">
          ...
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span key={i} className={`w-2 h-2 rounded-full transition-all duration-300 ${i === centerIndex ? "bg-cyan-400 scale-[1.3]" : "bg-white/20"}`} />
            ))}
          </div>
